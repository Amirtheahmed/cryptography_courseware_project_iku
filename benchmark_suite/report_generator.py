...
def generate_report(inputs):
    """
    inputs: List of tuples [(label, filepath), ...]
    """
    aggregated_data = []
    ...
    for label, filepath in inputs:
        ...
        with open(filepath, 'r') as f:
            try:
                ...
                print(f"Hata: Geçersiz JSON formatı -> {filepath}")

    if not aggregated_data:
        print("Rapor oluşturmak için geçerli veri bulunamadı.")
        return

    # HTML İçeriğini Oluştur
    html_content = HTML_TEMPLATE.replace("DATA_PLACEHOLDER", json.dumps(aggregated_data))

    filename = "crypto_benchmark_report.html"
    with open(filename, "w", encoding='utf-8') as f:
        f.write(html_content)

    print(f"\n[BAŞARILI] Rapor oluşturuldu: {os.path.abspath(filename)}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kripto Karşılaştırma Raporu Oluşturucu")
    parser.add_argument('--env', action='append', help="Format: Etiket=dosyayolu (Örn: IoT=iot.json)", required=True)

    args = parser.parse_args()

    # Girdileri ayrıştır: "Label=Path" -> ("Label", "Path")
    parsed_inputs = []
    for item in args.env:
        if '=' in item:
            ...
            print(f"Geçersiz format (--env): {item}")

    generate_report(parsed_inputs)